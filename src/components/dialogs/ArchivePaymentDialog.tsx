...
import { archivePayment, type PaymentRecord } from '@/services/paymentsService';
import type { ApiError } from '@/services/apiClient';
import { useUiStore } from '@/store/uiStore';

interface ArchivePaymentDialogProps {
  open: boolean;
  onClose: () => void;
...
const ArchivePaymentDialog: React.FC<ArchivePaymentDialogProps> = ({
  open,
  onClose,
  payment,
  onSuccess
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      await archivePayment(payment.id, password);
      addToast({
        id: crypto.randomUUID(),
        message: 'Płatność została zarchiwizowana',
        severity: 'success'
      });
      onSuccess?.();
      handleClose();
      ...
      const message = apiError?.message || 'Wystąpił błąd podczas archiwizacji płatności';
  ...
  const step1Content = (
    <Box sx={{ p: 3 }}>
      ...
      >
        Zarchiwizowana płatność nie będzie widoczna na liście. Możesz ją przywrócić w dowolnym
        momencie.
      </Typography>
  ...
  const step2Content = (
    <Box sx={{ p: 3 }}>
      ...
      >
        ...
        >
          Potwierdź archiwizację płatności
        </Typography>
        <IconButton onClick={handleClose} size="small" sx={{ color: 'rgba(0,0,0,0.54)' }}>
          <CloseIcon />
        </IconButton>
      ...
      >
        Podaj hasło używane do logowania do systemu, aby zarchiwizować płatność
      </Typography>
...
export default ArchivePaymentDialog;
